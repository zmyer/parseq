...
public class TestJsonTraceCodec
{
  ...
  {
    final Map<String, String> childAttrs = Collections.singletonMap("foo", "bar");
    ...
    final String json = buildJson(snapshotNanos,
                                  new String[] {traceStr(1, "parent", ResultType.UNFINISHED, false),
                                                traceStr(2, "child", ResultType.UNFINISHED, false, childAttrs),
    ...
    {
      throw new IOException("JSON parse failed. Document:\n" + json + "\nError: " + e.toString(), e);
    }
    ...
    assertEquals(childAttrs, trace.getShallowTrace(2).getAttributes());
  }
  ...
  {
    return traceStr(id, name, resultType, hidden, Collections.<String, String>emptyMap());
  }

  private String traceStr(final int id, final String name, final ResultType resultType, final boolean hidden, final Map<String, String> attributes)
  {
    String attrJson = "";
    if (!attributes.isEmpty())
    {
      attrJson = ", " + quote(JsonTraceCodec.TRACE_ATTRIBUTES) + ": [";
      for (Iterator<Map.Entry<String, String>> it = attributes.entrySet().iterator(); it.hasNext(); )
      {
        attrJson += "{ ";

        final Map.Entry<String, String> entry = it.next();
        attrJson += quote(JsonTraceCodec.TRACE_ATTRIBUTE_KEY) + ": " + quote(entry.getKey()) + ", ";
        attrJson += quote(JsonTraceCodec.TRACE_ATTRIBUTE_VALUE) + ": " + quote(entry.getValue());
    ...
    return "{" +
              ...
              quote(JsonTraceCodec.TRACE_RESULT_TYPE) + ": " + quote(resultType.toString()) +
              attrJson +
           "}";
  }
