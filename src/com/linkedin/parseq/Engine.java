...
public class Engine {
    ...
    /* package private */
    Engine(final Executor taskExecutor, final DelayedExecutor timerExecutor,
    ...
    private TaskQueueFactory createTaskQueueFactory(final Map<String, Object> properties,
            final TaskQueueFactory taskQueueFactory) {
            ...
            if (_properties.containsKey(DEFAULT_TASK_QUEUE)) {
                ...
                try {
                    final Class<? extends SerialExecutor.TaskQueue> clazz =
                            (Class<? extends TaskQueue>) Thread.currentThread().getContextClassLoader().loadClass(
                                    className);
                    return () -> {
                        try {
                    ...
                    LOG.error("Failed to load TasQueue implementation: " + className +
                            ", will use default implementation", e);
                }
            }
    ...
    public void run(final Task<?> task, final String planClass) {
        if (!tryRun(task, planClass)) {
            throw new IllegalStateException(
                    "Starting new plan rejected, exceeded limit of concurrent plans: " + _maxConcurrentPlans);
        }
    }
     ...
     * been {@linkplain Thread#interrupt interrupted}.
     */
    public boolean tryRun(final Task<?> task, final String planClass, final long timeout, final TimeUnit unit)
