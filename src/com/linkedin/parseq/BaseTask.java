  ...
  private boolean transitionDone(T value, Throwable error, boolean cancellation)
  {
    ...
    do
    {
      ...
      if (error == null)
      {
        ...
        stateBuilder.setValue(value != null ? value.toString() : null);
      }
      else
      {
        if (error instanceof EarlyFinishException)
        {
          stateBuilder.setResultType(ResultType.EARLY_FINISH);
        }
        else
        {
          stateBuilder.setResultType(ResultType.ERROR);
          stateBuilder.setValue(error.toString());
        }
