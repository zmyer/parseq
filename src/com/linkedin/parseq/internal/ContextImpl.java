...
public class ContextImpl implements Context, Cancellable {
    ...
    // A thread local that holds the root task if the root task is being executed
    ...
    public void runTask() {
        ...
        _task.addListener(resolvedPromise -> {
            for (final Iterator<Cancellable> it = _cancellables.iterator(); it.hasNext(); ) {
    ...
    public Cancellable createTimer(final long time, final TimeUnit unit, final Task<?> task) {
        checkInTask();
        final Cancellable schedule = _planContext.schedule(time, unit, new Runnable() {
            @Override
            public void run() {
                runSubTask(task, NO_PREDECESSORS);
            }
        });
        final Cancellable cancellable = schedule;
